...
class HotelAPI:
    def __init__(self):
        ...
        self.places_api = StructuredTool.from_function(
            ...
            name="Places API Tool",
            description="This tool allows you to query the Places API and get the results back. You can find restaurants, hotels, and other places of interest near a given location. pass Address to get the Points of Interest near that location.",
        )
    ...
    def get_poi(self, address: str):
        x = self.agent_executor.invoke(
            {
                "input": f"""give me POI at Address: {{{address}}}. Use the Places API Tool. Return the name, latitude, and longitude of the POI."""
            }
        )
        return x["output"]


# print(HotelAPI().get_poi("Railway Station Hyderabad, India"))
